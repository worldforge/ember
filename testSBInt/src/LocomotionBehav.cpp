...
<(W.W)> understand locomotionSetup, startingSetup, idleTurnSetup, transitionSetup and addJointTrajectoryConstraint (IK constraints) 

*/

static std::string defaultMotionPath(".");


LocomotionBehav::LocomotionBehav(std::string const &motionPath /*= ""*/, std::string const &skeletonRef /*= ""*/)
	: Behaviors::Behaviors(motionPath, skeletonRef)
{
}

LocomotionBehav::~LocomotionBehav(void)
{

}


void LocomotionBehav::setDefaultMotionPath(std::string const &path)
{
	defaultMotionPath = createDefaultMotionPath(path);
}


/* Create the build joint trajectories for this motion. */
void LocomotionBehav::setupMotion(SmartBody::SBMotion *motion)
{
...
void LocomotionBehav::setupRetargeting(SmartBody::SBCharacter *character)
{
	locomotionSetup();
	startingSetup();
	idleTurnSetup();
...
/* Get the names of all locomotion motions. */
std::vector<std::string> LocomotionBehav::getMotions(void)
{
...
/* Get the names of the motions involved in locomotion setup. */
std::vector<std::string> LocomotionBehav::getMotionsForLocomotionSetup(void)
{	
	std::vector<std::string> motions;
...
void LocomotionBehav::assignDefaultMotionPath(void)
{	
	mMotionPath = defaultMotionPath;
}

void LocomotionBehav::assignSkeletonReference(void)
{
	mSkelRefName = "Utah.sk";
}


void LocomotionBehav::locomotionSetup()
{	
	...
	/* We create a new stateManager refering to the original skeleton. */
	...
	SmartBody::SBSkeleton *skeleton = scene->getAssetManager()->getSkeleton(mSkelRefName);
	...
	for (int i = 0; i < size; i ++)
	{
		motion = scene->getAssetManager()->getMotion(motions[i]);
...
void LocomotionBehav::startingSetup()
{
	std::cout << "LocomotionBehav::startingSetup" << std::endl;
	
	SmartBody::SBScene *scene = SbManager::getSbScene();

	/* We generate the mirror animations for StopToWalk, Idle_ToWalk_Turn90 and Idle_ToWalk_Turn 180. */
	...
	/* Set the blend manager for each side. */
	for (int i = 0; i < 2; i ++)
	{
...
void LocomotionBehav::idleTurnSetup()
{
	std::cout << "LocomotionBehav::idleTurnSetup" << std::endl;
...
void LocomotionBehav::stepSetup()
{
	std::cout << "LocomotionBehav::stepSetup" << std::endl;
	...
	SmartBody::SBAssetManager *assetMng = SbManager::getSbScene()->getAssetManager();
	SmartBody::SBSkeleton *skeleton = assetMng->getSkeleton(mSkelRefName);
	SmartBody::SBJoint *joint = skeleton->getJointByName("base");
	...
	for (int i = 0; i < 7; i ++)
	{
		motion = assetMng->getMotion(motions[i]);
		motion->connect(skeleton);
...
void LocomotionBehav::transitionSetup()
{
	std::cout << "LocomotionBehav::transitionSetup" << std::endl;
