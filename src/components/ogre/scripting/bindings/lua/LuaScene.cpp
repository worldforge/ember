...
template <>
void registerLua<Scene>(sol::table& space) {
	auto scene = space.new_usertype<Scene>("Scene", sol::no_constructor);
	scene["getSceneManager"] = &Scene::getSceneManager;
	scene["getMainCamera"] = &Scene::getMainCamera;

	auto screen = space.new_usertype<Screen>("Screen", sol::no_constructor);
